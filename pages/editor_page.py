...
def add_caption(browser):
    ...
    if len(caption) >= 2:
        ...
        do_click(browser, CLICK_ADD_BUTTON)
        do_click(browser, CAPTION_OPTION)
        caption3 = browser.find_elements(HOVER_CAPTION[0], HOVER_CAPTION[1])
        assert len(caption3) >= 2
        do_click(browser, SECOND_CAPTION)
        change_text_color_and_bg(browser)
...
def change_font(browser):
    """
    Changes font in editor
    This one verifies Lobster font
    to_improve: avoid hardcoded font name, add a font name as a parameter in function
    """
    is_visible(browser, FONTS_DROPDOWN)
    do_click(browser, FONTS_DROPDOWN)
    do_click(browser, SEARCH_EMPTY_INPUT)
    do_send_keys(browser, SEARCH_FONT_INPUT, "Lobster")
...
def upload_logo(browser):
    ...
    if is_visible(browser, EDITOR_LOGO_FILLED) is True:
        do_double_click(browser, EDITOR_LOGO_FILLED)
        logos = browser.find_elements_by_xpath(UPLOADED_LOGOS_HOV[1])
        logos_len = len(logos)
        for x in range(1, logos_len + 1):
            do_hover(browser, UPLOADED_LOGOS_HOV)
        ...
        do_click(browser, CROSS_BTN)
        time.sleep(2)
        if is_visible(browser, EDITOR_LOGO) is True:
            do_click(browser, EDITOR_LOGO)
        elif is_visible(browser, EDITOR_LOGO_NEW) is True:
            do_click(browser, EDITOR_LOGO_NEW)
        elif is_visible(browser, EDITOR_LOGO_FILLED) is True:
            do_double_click(browser, EDITOR_LOGO_FILLED)
        else:
            assert is_visible(browser, EDITOR_LOGO_FILLED_SELECTED_TS) is True
            do_double_click(browser, EDITOR_LOGO_FILLED_SELECTED_TS)
        do_send_keys(browser, UPLOAD_LOGO, os.getcwd() + brand_logo)
        if is_visible(browser, IMAGE_DONE_BTN, 10):
            ...
            pass
        do_click(browser, ADD_TO_VIDEO_BTN)
        if is_visible(browser, EDITOR_LOGO_FILLED_SELECTED, 25) is True:
            try:
                assert is_visible(browser, EDITOR_LOGO_FILLED_SELECTED, 25) is True
            except AssertionError:
                assert is_visible(browser, EDITOR_LOGO_FILLED) is True
        else:
            assert is_visible(browser, EDITOR_LOGO_FILLED_SELECTED_TS) is True
    else:
        if is_visible(browser, EDITOR_LOGO) is True:
...
def filter_editorial_media_photos_search(browser):
    ...
    filtered_photos = browser.find_elements_by_xpath(FILTERED_PHOTOS[1])
    print(len(filtered_photos))
    no_of_videos = len(filtered_photos)
...
def purchase_plan_for_watermark_upload(browser):
    is_visible(browser, EDITOR_WATERMARK)
    do_click(browser, EDITOR_WATERMARK)
    assert is_visible(browser, EDITOR_WATERMARK_PRICING_POPUP) is True
    is_visible(browser, editor_pro_plan)
    currencies_check(browser)
    do_click(browser, editor_pro_plan)
    if os.environ['url'] == 'https://promo.com':
        assert is_visible(browser, EDITOR_PAYMENT_CARD_IFRAME_NEW, 10) is True
        switch_to_iframe(browser, EDITOR_PAYMENT_CARD_IFRAME_NEW, 10)
        if is_visible(browser, CHECKOUT_POP_UP_CLOSE2):
            do_click(browser, CHECKOUT_POP_UP_CLOSE2)
        else:
            do_click(browser, CHECKOUT_POP_UP_CLOSE)
        browser.switch_to.default_content()
        time.sleep(2)
        do_click(browser, EDITOR_PRICING_POP_UP_CLS3)
    else:
...
def go_back_to_editor_then_discard(browser):
    ...
    is_visible(browser, RATIO_TOOLBAR, 40) is False
...
def check_pricing_new_user(browser):
    ...
    """
    if is_visible(browser, PUBLISH_READY, 380) is True or is_visible(browser, CREATE_POST_BTN, 380) is True:
        if is_visible(browser, CREATE_POST_BTN) is True:
            do_click(browser, CREATE_POST_BTN)
        else:
