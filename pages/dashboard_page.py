...
def open_dashboard_published_tab(browser):
    ...
    to_delete: else part to delete when COR1-3981 is done
    """
    if is_visible(browser, MY_VIDEOS) is True:
        do_hover(browser, MY_VIDEOS)
        time.sleep(4)
        do_click(browser, MY_VIDEOS_PUBLISHED)
    else:
        is_visible(browser, MY_VIDEOS_PLANNER)
        do_hover(browser, MY_VIDEOS_PLANNER)
        time.sleep(4)
        do_click(browser, MY_VIDEOS_PUBLISHED_PLANNER)
